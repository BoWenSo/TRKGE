...
class ComplEx(TKBCModel):
    ...
    def forward(self, x):
        ...
        return (
                       ...
                       (lhs[0] * rel[1] + lhs[1] * rel
                       [0]) @ right[1].transpose(0, 1)
               ), (
...
class TRKGE(TKBCModel):
    def __init__(
            self, sizes: Tuple[int, int, int, int], rank: int,
            no_time_emb=False, init_size: float = 1e-2, base_time: bool = False
    ):
        super(TRKGE, self).__init__()
        self.sizes = sizes
        self.rank = rank
        ...
        self.context = nn.Embedding(self.sizes[1], int(self.rank))
        self.context.weight.data = self.init_size * torch.randn((self.sizes[1], int(self.rank)),
                                                                         dtype=torch.float32)

        self.scale = torch.Tensor([1. / np.sqrt(self.rank)]).cuda()


    def att(self, head, rela, time, x):

        rot_q = givens_rotations(rela, head)

        rot_time = givens_rotations(time, head)

        con_vec = self.context(x[:,1]).view((-1, 1, self.rank))

        cands = torch.cat(
            [rot_q.view((-1, 1, self.rank)), rot_time.view((-1, 1, self.rank))], dim=1)
        att_weights = torch.sum(con_vec * cands * self.scale, dim=-1, keepdim=True)
    ...
    def forward(self, x):
        ...
        att_q1 = self.att(lhs[0], rel[0], time[0], x)
        att_q2 = self.att(lhs[1], rel[1], time[1], x)
        ...
        return ((
                        (q1 * full_rel[0] - q2 * full_rel[1]) @ right[0].t() +
                        (q2* full_rel[0] + q1 * full_rel[1]) @ right[1].t()
                ), regularizer,
    ...
    def score(self, x):
        ...
        return torch.sum(
            (q1 * full_rel[0] - q2  * full_rel[1]) * rhs[0] +
            (q2  * full_rel[0] + q1 * full_rel[1]) * rhs[1],
            1, keepdim=True
        )
    ...
    def get_queries(self, queries: torch.Tensor):
        ...
        att_q1 = self.att(lhs[0], rel[0], time[0], queries)
        att_q2= self.att(lhs[1], rel[1], time[1], queries)
        q1= att_q1-att_q2
        q2 = lhs[1]
        ...
        return torch.cat([
            q1 * full_rel[0] - q2 * full_rel[1],
            q2  * full_rel[0] + q1 * full_rel[1]
        ], 1)
