...
class TemporalDataset(object):
    ...
    def get_rel_example(self,rel):
        rel_ex = []
        exam = np.concatenate((self.data['train'],self.data['valid'],self.data['test']),axis=0)
        for ex in exam:
            if ex[1] == rel:
                rel_ex.append(ex.astype('int64'))
        return rel_ex

    def count_rel(self):
        exam = np.concatenate((self.data['train'],self.data['valid'],self.data['test']),axis=0)
        re = np.zeros((int(self.n_predicates/2)))
        for ex in exam:
            re[ex[1]]+=1

        file = "./attention/rel_count.txt"
        f = open(file, "w")
        for i in range(int(self.n_predicates/2)):
            f.write(str(re[i]) + '\n')
    ...
    def eval_rel(
            self, model: TKBCModel, split: str, n_queries: int = -1, rel: int =-1, missing_eval: str = 'both',
            at: Tuple[int] = (1, 3, 10)
    ):
        if self.events is not None:
            return self.time_eval(model, split, n_queries, 'rhs', at)
        ...
        for exa in examples:
            if exa[1] == rel:
                rel_example.append(exa)


        for m in missing:
            q = rel_example.clone()
            if n_queries > 0:
                permutation = torch.randperm(len(rel_example))[:n_queries]
                q = rel_example[permutation]
            if m == 'lhs':
                tmp = torch.clone(q[:, 0])
                q[:, 0] = q[:, 2]
                q[:, 2] = tmp
